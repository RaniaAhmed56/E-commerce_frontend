...
export default function AdminEditProduct() {
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    if (validV.length===0){setError("أضف لوناً واحداً على الأقل");return;}
    for (const v of validV) {
      if (v.sizes.length===0){setError(`اللون "${v.color}" ليس له مقاسات`);return;}
      if (v.sizes.some(s=>!s.size.trim())){setError(`اكتب اسم المقاس لكل مقاس في "${v.color}"`);return;}
    }
    ...
    try {
      ...
      const msg = Object.values(apiError?.data??{}).flat().join(" ")||"حدث خطأ عند الحفظ";
      setError(String(msg));
  ...
  if (!product) return (
    <div style={{textAlign:"center",padding:"60px 24px"}}>
      ...
      <p style={{color:"rgba(255,255,255,0.4)",fontSize:14}}>المنتج غير موجود</p>
    ...
    <div>
      ...
      {saved && (
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",background:"rgba(16,185,129,0.1)",border:"1px solid rgba(16,185,129,0.25)",borderRadius:10,marginBottom:20}}>
          <p style={{fontSize:13,color:"#34d399",margin:0}}>✅ تم حفظ التعديلات بنجاح</p>
        </div>
      )}
        ...
        <div className="add-product-grid">
                ...
                {variants.map((variant,vi)=>(
                  <div key={vi} style={{border:"1.5px solid rgba(255,255,255,0.08)",borderRadius:14,overflow:"hidden",background:"rgba(255,255,255,0.02)"}}>
                    <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:"rgba(255,255,255,0.04)",borderBottom:variant.expanded?"1px solid rgba(255,255,255,0.07)":"none"}}>
                      ...
                      <span style={{flex:1,fontSize:13,fontWeight:700,color:variant.color?"#ffffff":"rgba(255,255,255,0.3)"}}>{variant.color||`اللون ${vi+1}`}</span>
                      ...
                      <div style={{padding:"16px"}}>
                        <div style={{marginBottom:14}}>
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:10}}>
                            ...
                            <span style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>أو اختر من الألوان الجاهزة:</span>
                          </div>
                        ...
                        <div style={{marginBottom:16}}>
                          ...
                          <label htmlFor={`var-img-${vi}`} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",border:"1.5px dashed rgba(255,255,255,0.15)",borderRadius:10,cursor:"pointer",background:"rgba(255,255,255,0.04)",fontSize:12,color:"rgba(255,255,255,0.5)"}}>
                            <ImageIcon size={16}/> {variant.imagePreview||variant.imageUrl?"تغيير الصورة":"اختر صورة لهذا اللون"}
                          </label>
                          ...
                          <div style={{display:"flex",flexDirection:"column",gap:8}}>
                            {variant.sizes.map((sz,si)=>(
                              <div key={si} style={{display:"grid",gridTemplateColumns:"1fr 100px 32px",gap:8,alignItems:"center"}}>
                                <input type="text" value={sz.size} onChange={e=>updateSize(vi,si,{size:e.target.value})} placeholder="المقاس" className="input-field" style={{fontSize:13,padding:"8px 12px"}}/>
                                <input type="number" min="0" value={sz.quantity} onChange={e=>updateSize(vi,si,{quantity:parseInt(e.target.value)||0})} placeholder="الكمية" className="input-field" style={{fontSize:13,padding:"8px 12px"}}/>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:18}}>
            <div className="admin-card">
              ...
              <div style={{display:"flex",flexDirection:"column",gap:16}}>
                {[{k:"in_stock",l:"متوفر",desc:"يظهر للشراء"},{k:"featured",l:"مميز",desc:"يظهر في المميز"},{k:"trending",l:"رائج",desc:"يظهر في الرائج"}].map(f=>(
              ...
              {variants.filter(v=>v.color).map((v,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <p style={{fontSize:11,color:"rgba(255,255,255,0.35)",margin:0}}>{v.sizes.length} مقاس · {v.sizes.reduce((s,x)=>s+Number(x.quantity),0)} قطعة</p>
                  </div>
                </div>
              ))}
            </div>
            <button type="submit" disabled={saving} className="btn-admin" style={{width:"100%",justifyContent:"center",fontSize:13,padding:"14px 24px"}}>
              {saving?<><Loader2 size={14} style={{animation:"spin 1s linear infinite"}}/> جارٍ الحفظ...</>:<><Save size={15}/> حفظ التعديلات</>}
            </button>
